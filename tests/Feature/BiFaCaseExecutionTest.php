...
test('every executable bifa case reproduces all of its declared routes through the pan page', function () {
    ...
    foreach (['bifa.01', 'bifa.02'] as $lawCode) {
        foreach (BiFaCaseCatalog::casesForLaw($lawCode) as $case) {
            ...
            $bifa = collect(app(BiFaRuleEngine::class)->evaluate(new PanResult($component->get('pan'))))
                ->first(fn ($match): bool => $match->code === $lawCode);

            if ($bifa === null) {
                $failures[] = "case_id={$case['case_id']}: 起盘后 {$lawCode} 必须命中";
