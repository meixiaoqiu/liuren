...
- `tools/` 只用于可复用、需要纳入版本管理的正式工具和脚本，不得写入测试输出、Pint 输出、调试日志、临时 diff、搜索结果或一次性报告。
- 需要落盘的开发临时产物统一写入 `storage/app/private/dev/`；该目录可按需创建，并由 `storage/app/private/.gitignore` 统一排除，不进入 Git。
