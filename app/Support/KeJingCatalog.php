    ...
    public static function lessons(): array
    {
        return [
            ...
            [
                ...
                'gua' => '困',
                'guaSymbol' => '䷮',
                'summary' => '日鬼、日干墓神或日支墓神发用。三条入口「日鬼 / 日干墓 / 日支墓」是并列 OR，初传同时是鬼与墓时显示「鬼墓兼见（鬼墓俱见）」增强判断。',
                'cases' => [
                    self::case(
                        'lesson.guimu.day_ghost_only',
                        '乙亥日·卯时·丑将（仅日鬼发用）',
                        '日干乙对应正文日鬼为酉，初传酉为日鬼发用；初传非日干墓未、非日支墓辰，仅由日鬼路线独立命中。',
                        '2026-01-01T05:00',
                    ),
                    self::case(
                        'lesson.guimu.stem_tomb_only',
                        '乙亥日·巳时·丑将（仅日干墓发用）',
                        '日干乙属木，五行墓在未，初传未为日干墓发用；初传非日鬼酉、非日支墓辰，仅由日干墓路线独立命中。',
                        '2026-01-01T09:00',
                    ),
                    self::case(
                        'lesson.guimu.branch_tomb_only',
                        '乙亥日·丑时·丑将（仅日支墓发用）',
                        '日支亥属水，五行墓在辰，初传辰为日支墓发用；初传非日鬼酉、非日干墓未，仅由日支墓路线独立命中。',
                        '2026-01-01T01:00',
                    ),
                    self::case(
                        'lesson.guimu.ghost_tomb_combined',
                        '壬午日·申时·丑将（鬼墓兼见·壬日辰发用）',
                        '日干壬对应正文日鬼为辰、戌；日干壬属水，五行墓在辰。初传辰同时是日鬼与日干墓，构成《六壬大全》正文所示「壬日辰发用」鬼墓兼见结构。',
                        '2026-01-08T15:00',
                    ),
                    self::case(
                        'lesson.guimu.all_three',
                        '壬子日·戌时·丑将（三路线同时命中）',
                        '日干壬对应正文日鬼为辰、戌；日干壬属水，五行墓在辰；日支子属水，五行墓亦在辰。初传辰同时是日鬼、日干墓与日支墓，三条入口全部成立。',
                        '2026-02-07T19:00',
                    ),
                ],
                'source_examples' => [
                    ['label' => '壬日辰发用', 'path' => '鬼墓兼见（鬼墓俱见）', 'source' => '《六壬大全》正文', 'detail' => '辰为壬日鬼、亦为壬水五行墓，辰发用即鬼墓兼见。现代复现时间为 2026-01-08 15:00。'],
                    ['label' => '鬼入传', 'path' => '日鬼发用', 'source' => '《六壬大全》正文', 'detail' => '凡日鬼发用即成鬼墓课主体；日鬼依本课列出的同性相克表，不作普通五行相克。'],
                    ['label' => '传墓', 'path' => '日干墓或日支墓发用', 'source' => '《六壬大全》正文', 'detail' => '凡日干墓神或日支墓神发用亦成鬼墓课主体；墓法以五行墓为准（未、戌、辰、丑），不用十干十二长生墓。'],
                    ['label' => '支鬼支墓', 'path' => '《订讹》严格解释', 'source' => '《订讹》', 'detail' => '《订讹》进一步要求「干支上神发用并兼为鬼墓」，并提出支鬼支墓；因支鬼尚无稳定古籍程序定义，本课未并入正式 matcher。'],
                ],
            ],
        ];
    }
