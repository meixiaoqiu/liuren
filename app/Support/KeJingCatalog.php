        ...
        return [
            ...
            [
                ...
                'cases' => [
                    self::case(
                        ...
                        '《六壬大全》"戊申日子时申将"课例的现代复现（部分复原）：戊申日子时（00:00）、月将申（节气对应芒种），三传子申辰，初传子与干上丑作子丑六合；日支申、初传子、末传辰作申子辰三合，初传明确参与"支三合发用"。干上丑在甲辰旬中寄宫癸，与日干戊作戊癸合。1959-06-21T12:00 男（己亥年生人）为现代补充资料，本命=亥，盘亥上神=未（贵人）✓，行年=未（本项目 FateCalculator 公式下），盘未上神=卯（太常）✓，两者均乘六吉将。重要诚实边界：课例原文"行年在辰见子为青龙"在项目男一岁起丙寅公式下与"亥命"不可同时成立（2000 年 1959 己亥年男行年=未），故此案例仅能"部分复原"年命结构。',
                        '2000-06-19T00:00',
                        '1959-06-21T12:00',
                        'male',
                    ),
            ...
            [
                ...
                'cases' => [
                    self::case(
                        ...
                        '《六壬大全》"戊子日戊时卯将占"课例的现代复现：戊子日（4/0）、亥月（11）、戌时，卯将，三传巳戌卯，初传巳同时为戊日干德与子日支德。起课时间复原《大全》原盘式；1984-06-01 00:00 出生 + 男性为现代补充资料（不是古籍原例所载人物），用于让本命宫上神 = 巳 = 初传：1984 为甲子年，本命=子；地盘子位上神=巳（恰好等于初传），初传巳乘太阴（六吉将），故通过本命路径严格命中德庆课。',
                        '2001-11-21T19:00',
                        '1984-06-01T00:00',
                        'male',
                    ),
            ...
            [
                ...
                'source_examples' => [
                    ...
                    ['label' => '乙酉日', 'detail' => '三传申子辰水局生日（乙日 YIN=水），支上丑作六合，并三合为全吉。'],
            ...
            [
                ...
                'gua' => '遁',
                'guaSymbol' => '䷠',
                'summary' => '天魁（戌）或天罡（辰）发用为初传，且该发用之神加临日干寄宫（天盘日干位上神为该神）或加临日支（天盘日支位上神为该神）。《六壬大全》卷八《课经集（二）·斩关课》（识典底本 130 节）给出"如甲寅日亥时未将占，戌加寅为用，曰斩关课"为唯一正文课例；本目录首选案例为 1904-07-19 21:00（甲寅日亥时·小暑后月将未·三传戌午寅·戌加寅为用），由 PanCalculator 在 1900-2100 全年范围搜索命中。注意：甲寅日因甲干寄寅=日支寅位置重合，"戌加寅"同时命中加日干与加日支两路径，不能单独证明 OR 解读；OR 主体目前依据"日辰"的通常并列语义维持。',
                'cases' => [
                    self::case(
                        'lesson.zhan_guan.jia_yin_hai_shi_wei_jiang',
                        '甲寅日·亥时·未将（识典底本唯一课例·现代复现）',
                        '《六壬大全》卷八《课经集（二）·斩关课》（识典底本 130 节）正文课例"如甲寅日亥时未将占，戌加寅为用，曰斩关课"的现代真实时间复现。在 1900-2100 全年范围搜索"甲寅日 + 亥时 + 月将=未"后，命中 105 例；1904-07-19 21:00:00 为搜索命中的最早一例，且完整满足原文四要素：日柱=甲寅、时支=亥、月将=未、初传=戌，三传=戌午寅，tianpan[寅]=戌，命中斩关课主体。本案例 datetime 早于默认 birth (1986-08-01)，故显式给出早于 datetime 的 birth 资料；斩关课主体不依赖人物上下文，birth 仅用于满足排盘页校验。',
                        '1904-07-19T21:00',
                        '1900-01-01T00:00',
                    ),
                    self::case(
                        'lesson.zhan_guan.yi_hai_zi_shi',
                        '乙亥日·子时（OR 路径测试例，非文献证据）',
                        '2026-01-01 01:00 乙亥日子时：日干乙寄宫辰，地盘辰位天盘上神 = 辰（伏吟），初传辰 = 天罡 = 干上神，构成"天罡加临日干寄宫"，命中斩关课主体。本案例**仅用于测试 OR 公式独立命中"加日干"路径**，**不是古籍文献证据**，不能反过来证明 OR 是唯一正解。',
                        '2026-01-01T01:00',
                    ),
                ],
                'source_examples' => [
                    ...
                    ['label' => '凡见辰戌加日辰发用者', 'source' => '《观月经》（识典底本 130 节附录）', 'detail' => '凡见辰戌加日辰发用者，为斩关卦，必有逃走之应也。注：此句未明确"加日辰"是 OR 还是 AND；后接"戊申正月占，酉时此卦攻"诗句，按正月亥将、酉时还原天地盘并由 720 fixture 对应 pointer 核验，得"干上未、支上戌、三传子寅辰"（初传=子，非辰戌），提示《观月》可能采用比正文更宽的异说（仅要求日辰上见辰戌，不要求初传=辰/戌），主体不并入此异说。'],
                ],
            ],
        ];
    }
